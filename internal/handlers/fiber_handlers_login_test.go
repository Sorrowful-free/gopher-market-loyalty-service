...
func TestLoginHandler(t *testing.T) {
	app := fiber.New()
	app.Post(LoginUserPath, Login)
	app.Listen(":3000")

	t.Run("successful_login", func(t *testing.T) {
		req := httptest.NewRequest(fiber.MethodPost, LoginUserPath, bytes.NewBuffer([]byte(TestLoginJSON)))
	...
	t.Run("failed_login_with_wrong_format", func(t *testing.T) {
		req := httptest.NewRequest(fiber.MethodPost, LoginUserPath, bytes.NewBuffer([]byte(TestLoginText)))
